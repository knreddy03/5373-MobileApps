...
class MongoManager:
    ...
    def add_location(self, email, latitude, longitude, timestamp):
        """
        Inserts or updates a user's location in the 'locations' collection as a GeoJSON point.
    
        :param email: The email address of the user.
        :param latitude: The latitude of the location.
        :param longitude: The longitude of the location.
        :param timestamp: The timestamp when the location was recorded.
        :return: Dictionary indicating the success status of the operation.
        """
        try:
            # Ensure the collection is set to 'locations'
            self.setCollection("locations")
        
            # Create GeoJSON point
            location = {"type": "Point", "coordinates": [longitude, latitude]}
        
            # Location data to insert or update
            location_data = {
                "email": email,
                "location": location,
                "timestamp": timestamp
            ...
            # Use upsert to ensure only one location per user
            result = self.collection.update_one(
                {"email": email},  # Query to find existing location by email
                {"$set": location_data},  # Update with the new data
                upsert=True  # Insert if it doesn't exist
            )
        
            return {"success": True, "message": "Location data updated successfully"}
        except PyMongoError as e:
            return {"success": False, "message": f"Error updating location data: {str(e)}"}
        

    def find_user_location(self, email: str):
    # Retrieve all locations for a specific user, sorted by timestamp
        return self.collection.find_one(
            {"email": email},
            {"_id": 0, "email": 1, "location": 1, "timestamp": 1, "created_at": 1},
        )


    def get_locations_within_radius(self, latitude, longitude, radius):
        try:
            # Ensure geospatial indexing
            if "location_2dsphere" not in self.collection.index_information():
            ...
            query = {
                "location": {
                    "$geoWithin": {
                        "$centerSphere": [
                            [longitude, latitude], 
                            radius / 6378100.0  # Convert radius to radians (Earth's radius)
                        ]
                    }
                }
            }
            results = self.collection.find(query)
            return list(results)  # Return a list of matching location documents
        except PyMongoError as e:
            print(f"Error retrieving locations within radius: {e}")
            return None  # Return None on error

        
    def save_image(self, file_path, metadata):
        try:
            result = self.collection.insert_one({
                "metadata": metadata,
                "path": file_path  # Store the file path instead of the whole image data
            })
            logging.info(f"Image saved: {file_path}, Metadata: {metadata}")
    ...
    def get_image(self, image_id):
        try:
                image_doc = self.collection.find_one({"_id": ObjectId(image_id)})
                if image_doc:
                        return {
                ...
                print(f"Error retrieving image from MongoDB: {str(e)}")
                return None


    def get_image(self, image_id):
        try:
                # Ensure that the image_id is a valid ObjectId
                image_doc = self.collection.find_one({"_id": ObjectId(image_id)})
    ...
    def get_all_images(self):
        # Ensure proper check for collection being set
        if self.collection is None:
            raise ValueError("Collection is not set. Use setCollection to set a collection.")

        try:
            images = list(self.collection.find())  # Retrieve all documents
